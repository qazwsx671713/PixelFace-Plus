...
class Con_PosFold(nn.Module):
    #merge method: concate
    def __init__(self, in_shape, out_shape, use_const=False,i=None,mask_dim = 3):
        super().__init__()
        in_c, in_h, in_w = in_shape
        out_c, out_h, out_w = out_shape
        if use_const:
            self.conv0 = ConvLayer(out_c*2+mask_dim, out_c, 1)
        if i==0:
            self.conv1 = ConvLayer(out_c, in_c*in_h*in_w//out_h//out_w, 1)
        elif i==1:
            self.conv1 = ConvLayer(out_c+mask_dim, in_c*in_h*in_w//out_h//out_w, 1)
        elif i==2:
            self.conv1 = ConvLayer(out_c+mask_dim, in_c*in_h*in_w//out_h//out_w, 1)
